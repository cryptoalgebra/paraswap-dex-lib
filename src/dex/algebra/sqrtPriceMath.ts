...
import { ONE, Q96, ZERO, MaxUint256 } from './internalConstants';
...
export abstract class SqrtPriceMath {
  ...
  public static getNextSqrtPriceFromInput(
    ...
    if (JSBI.LT(sqrtPX96, ZERO) || JSBI.LT(liquidity, ZERO)) return null;
  ...
  private static getNextSqrtPriceFromAmount0RoundingUp(
    ...
    if (add) {
      ...
      if (
        JSBI.NE(JSBI.divide(product, amount), sqrtPX96) ||
        JSBI.LT(numerator1, product)
      )
        return null;
  ...
  private static getNextSqrtPriceFromAmount1RoundingDown(
    ...
    if (add) {
      ...
      if (JSBI.LT(sqrtPX96, quotient)) return null;
